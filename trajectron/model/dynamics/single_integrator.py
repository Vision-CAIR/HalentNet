...
class SingleIntegrator(Dynamic):
    ...
    def integrate_samples(self, v, x=None):
        ...
        """
        p_0 = self.initial_conditions['pos'].unsqueeze(1)  # position of first step
        return torch.cumsum(v, dim=2) * self.dt + p_0  # integrate the velocity and add the first position. this function is for one sample instead of the whole distribution. differentiable
