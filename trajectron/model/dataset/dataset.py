...
class EnvironmentDataset(object):
    def __init__(self, env, state, pred_state, node_freq_mult, scene_freq_mult, hyperparams, pseudo=False, **kwargs):
        self.env = env
        self.state = state
        ...
        for node_type in env.NodeType:
            if node_type not in hyperparams['pred_state']:
                continue
            if pseudo:
                self.node_type_datasets.append(PseudoNodeTypeDataset(env, node_type, state, pred_state, node_freq_mult,
...
class PseudoNodeTypeDataset(NodeTypeDataset):
    def __init__(self, env, node_type, state, pred_state, node_freq_mult,
                 scene_freq_mult, hyperparams, augment=False, **kwargs):
        super().__init__(env, node_type, state, pred_state, node_freq_mult,
                 scene_freq_mult, hyperparams, augment, **kwargs)

    def index_env(self, node_freq_mult, scene_freq_mult, **kwargs):
        index = list()
        for scene in self.env.scenes:
            ts = scene.timesteps - kwargs['min_future_timesteps'] - 1
            index += [(scene, np.int64(ts), scene.pseudo_node)]
        return index
