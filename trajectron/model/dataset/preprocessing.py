...
def collate(batch):
    ...
    if elem is None:
        ...
        if len(elem) == 4: # We assume those are the maps, map points, headings and patch_size  # convert the map tuple in dataset to the map tensor in datatloader
...
def get_node_timestep_data(env, scene, t, node, state, pred_state,
    ...
    x = node.get(timestep_range_x, state[node.type])  # x is input, y is ground truth
    ...
    if hyperparams['use_map_encoding']:
        if node.type in hyperparams['map_encoder']:
            ...
            map_point = x[-1, :2]  # the last position in input data


            patch_size = hyperparams['map_encoder'][node.type]['patch_size']
            map_tuple = (scene_map, map_point, heading_angle, patch_size)  # scene map is the whole map. map point is the vehicle pos at last step. heading_angle is the vehicle head and patch_size is the size of map patch for this vehicle
