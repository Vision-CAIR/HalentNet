...
from environment.map import patch2img
...
class MultimodalGenerativeCVAE(object):
    ...
    def obtain_encoded_tensors(self,  # although it also takes unstandardized data, but only st is used.
                               mode,
                               inputs,
                               inputs_st,
                               labels,
    ...
    def p_y_xz(self, mode, x, x_nr_t, y_r, n_s_t0, z_stacked, prediction_horizon,
        ...
        for j in range(ph):
            ...
            gmm = GMM2D(log_pi_t, mu_t, log_sigma_t, corr_t)  # [k;bs, pred_dim]  # here gmm is just a gaussian (gmm with 1 component)

            if mode in [ModeKeys.PREDICT, ModeKeys.GAN] and gmm_mode:
                a_t = gmm.mode()
            else:
                a_t = gmm.rsample()
            ...
            corrs.append(
                ...
                ).permute(0, 2, 1).reshape(-1, num_components))  # #batch x #z_dim
            ...
            if self.hyperparams['incl_robot_node']:
                ...
                dec_inputs = [zx, a_t]  # the next action prediction is dependent on the current action sample
        ...
        a_dist = GMM2D(torch.reshape(log_pis, [num_samples, -1, ph, num_components]),
                       ...
                       torch.reshape(corrs, [num_samples, -1, ph, num_components]))  # this distribution is not deterministic! dependent on the action sample at each step
    ...
    def encoder(self, mode, x, y_e, num_samples=None):
        ...
        z = self.latent.sample_q(sample_ct, mode)  # it contains #batch 25-dim identity mat (don't actually sample here)
