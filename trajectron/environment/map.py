...
def patch2img(patch):
    return ((np.transpose(patch, (2, 1, 0))) * 255).astype(np.uint)
...
class GeometricMap(Map):
    ...
    def get_cropped_maps_from_scene_map_batch(cls, maps, scene_pts, patch_size, rotation=None, device='cpu'):
        ...
        centers = torch.tensor([s_map.to_map_points(scene_pts[np.newaxis, i]) for i, s_map in enumerate(maps)],
                  ...
                  + torch.tensor([context_padding_x, context_padding_y], device=device, dtype=torch.long)  # the location of scene point in the paded map
        ...
        center_patches = torch.tensor([[context_padding_y, context_padding_x]],  # the center of patch. patch size here is 2 * context_padding
                                      dtype=torch.int,
